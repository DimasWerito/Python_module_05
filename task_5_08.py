...
перший text та другий word. 
Функція виконує пошук зазначеного слова word у 
тексті text за допомогою функції search та повертає словник.
...
def find_word(text, word):
    ...
    response = re.search(word, text)
    if response:
        answer['result'] = True
        indexes = response.span()
        answer['first_index'] = indexes[0]
        answer['last_index'] = indexes[1]
    return answer
